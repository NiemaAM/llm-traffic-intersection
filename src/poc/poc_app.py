...
with st.sidebar:
    ...
    if use_llm:
        # ── Read from .env / environment automatically ────────────────────
        api_key    = os.environ.get("OPENAI_API_KEY", "")
        model_name = os.environ.get("MODEL_NAME", "gpt-4o-mini")
        fine_tuned = os.environ.get("FINE_TUNED_MODEL_ID", "")
        few_shot   = os.environ.get("FEW_SHOT", "true").lower() == "true"

        # Show status — no manual entry needed
        if api_key:
            ...
            st.caption("Add it to your `.env` file and restart the app.")
