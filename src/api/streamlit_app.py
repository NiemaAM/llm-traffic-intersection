...
with st.sidebar:
    ...
    try:
        ...
        st.caption("Start: `uvicorn src.api.app:app --port 8000`")
        st.caption("Or: `docker compose up`")
...
if len(st.session_state.vehicles) < 2:
    ...
    if st.button("🔍 Analyze Intersection", type="primary", use_container_width=True):
        ...
        with st.spinner("Consulting the LLM via FastAPI…"):
            try:
    ...
    API_TEST_CASES = [
        {
            ...
            "payload": {"vehicles": [
                ...
                {"vehicle_id": "T002", "lane": 1, "speed": 50,
        ...
        },
        {
            ...
            "payload": {"vehicles": [
                ...
                {"vehicle_id": "T004", "lane": 1, "speed": 20,
                 "distance_to_intersection": 500, "direction": "south", "destination": "C"},
            ]},
        ...
        for tc in API_TEST_CASES:
            ...
            if tc["payload"] is None:
                try:
                    ...
                    rows.append({
                        "Test": tc["name"], "Expected": "HTTP 200",
                        "Got": "ERROR", "Result": f"❌ {exc}", "Latency": "—",
                    })
                continue
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    rows.append({
                        "Test": tc["name"], "Expected": tc["expected_conflict"],
                        "Got": f"HTTP {r.status_code}", "Result": "❌ Fail",
                        "Latency": f"{ms} ms",
                    })
            except Exception as exc:
                rows.append({
